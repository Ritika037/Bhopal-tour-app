package com.CR.examples.android.bhopaldarshan.Data;
...
public class RestaurantData {
    ...
    public static ArrayList<Restaurant> fetchRestaurantData(Context context) {
        ...
        String[] about = resources.getStringArray(R.array.restoAbout);


        //Mapping data set with recycler view items accordingly
        for (int i = 0; i < restoImgId.length; i++) {
            Restaurant restaurant = new Restaurant(restoImgId[i], name[i], rating[i], type[i], phone[i], time[i], address[i], url[i], about[i]);
